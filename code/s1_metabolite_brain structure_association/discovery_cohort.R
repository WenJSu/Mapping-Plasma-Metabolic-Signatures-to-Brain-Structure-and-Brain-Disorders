...
imagedata <- fread('imgdata_aft_pro.csv', sep = ",", header = TRUE)

meta_index <- colnames(metadata)
image_index <- colnames(imagedata)
...
allSiteInfo <- allSiteInfo[,c('eid','54-2.0')]
allSiteInfo <- allSiteInfo[allSiteInfo$'54-2.0'!='11028',] # remove the site with too little subjects
allSiteInfo <- na.omit(allSiteInfo)
colnames(allSiteInfo)[which(colnames(allSiteInfo) == "54-2.0")] <- "Site"
...
covadata_used <- covadata[,c('eid','age2','Sex','Townsend_index','smoking2','drinking2','BMI2','Education','interval','eTIV')]
covadata_used <- covadata_used[covadata_used$smoking2!=-3 & covadata_used$drinking2!=-3,]
...
cov_all <- merge(cov_all,allSiteInfo,by='eid')
...
write.table(resultframe, 'glm_meta_IDP.csv',sep=',',row.names = F)
print("Finish!")
