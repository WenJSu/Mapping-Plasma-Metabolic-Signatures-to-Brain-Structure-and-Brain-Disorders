...
for(i in seq(1,length(diseases_list_use))){
  meta = diseases_list_use[i]
  print("**************************")
  print(i)
  print("**************************")
  ...
  exposure_dat_0 <- read_exposure_data(
    filename = paste0('finngen_R10_',meta),
    sep = "\t",
    snp_col = "rsids",
    beta_col = "beta",
    se_col = "sebeta",
  ...
  list_choose = use_data[use_data$diseases==meta,]
  diseases_choose = list_choose$meta
...
for(j in seq(1,length(diseases_choose))){
    ...
    if (dir.exists(outputpath)) {
        ...
        outcomefile <- paste0(out,'/GWAS_merge.glm.linear')
