...
for(i in seq(1,length(meta_list_use))){
  ...
  for(j in seq(231,nrow(IDP_df))){
    ...
    filepath = paste0(outputpath, out, "_",metaname,"_MR_IVW")
    cnt = 1
    ...
    ######################################### 3. MR analysis #########################################
    output1 <- paste0(outputpath, outcome,'_',metaname,'_MR_WM') #weight median
    output2 <- paste0(outputpath, outcome,'_',metaname,'_MR_IVW') #ivw
    output3 <- paste0(outputpath, outcome,'_',metaname,'_MR_egger') #egger slope
    output4 <- paste0(outputpath, outcome,'_',metaname,'_MR_W') #weight mode
    output5 <- paste0(outputpath, outcome,'_',metaname,'_MR_robustAdjProfile') #Robust adjusted profile score
    output6 <- paste0(outputpath, outcome,'_',metaname,'_MR_waldRatio') #Wald Ratio
    ...
    a1<-cbind(outcome, metaname,tsmr1$nsnp,tsmr1$b,(tsmr1$b-1.96*tsmr1$se),(tsmr1$b+1.96*tsmr1$se),tsmr1$pval) #weight median
    a2<-cbind(outcome, metaname,tsmr2$nsnp,tsmr2$b,(tsmr2$b-1.96*tsmr2$se),(tsmr2$b+1.96*tsmr2$se),tsmr2$pval) #ivw
    a5<-cbind(outcome, metaname,tsmr5$nsnp,tsmr5$b,(tsmr5$b-1.96*tsmr5$se),(tsmr5$b+1.96*tsmr5$se),tsmr5$pval) #raps
    a6<-cbind(outcome, metaname,tsmr6$nsnp,tsmr6$b,(tsmr6$b-1.96*tsmr6$se),(tsmr6$b+1.96*tsmr6$se),tsmr6$pval) #WR
    CI <- 0.95
    ...
    a3 <- cbind(outcome, metaname,tsmr3$nsnp,tsmr3$b, mapply(lowerCI, tsmr3$b, tsmr3$nsnp - 2, tsmr3$se), mapply(upperCI, tsmr3$b, tsmr3$nsnp - 2, tsmr3$se), tsmr3$pval) #egger slope
    a4 <- cbind(outcome, metaname,tsmr4$nsnp,tsmr4$b, mapply(lowerCI, tsmr4$b, tsmr4$nsnp - 1, tsmr4$se), mapply(upperCI, tsmr4$b, tsmr4$nsnp - 1, tsmr4$se), tsmr4$pval) #weight mode
